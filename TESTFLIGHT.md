# OneTap.kz TestFlight

The production app is configured with bundle identifier `kz.onetap.app` and
...
workflow defaults to `kz.onetap.beta` and `OneTap.kz Beta`. This preserves
the final `kz.onetap.app` identifier for the client's organization account;
...
`.p8` key. The key must belong to the Apple team that owns OneTap.kz and must
